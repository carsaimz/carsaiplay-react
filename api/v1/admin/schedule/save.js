...
export default async function handler(req) {
  ...
  const {content_id, release_date, episode_number} = await req.json();
  const [row] = await db`INSERT INTO schedule (content_id, release_date, episode_number, created_at) VALUES (${parseInt(content_id)}, ${release_date}, ${episode_number||null}, NOW()) RETURNING *`;
